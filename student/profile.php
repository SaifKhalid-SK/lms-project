...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['update_profile'])) {
    ...
    if (empty($full_name) || empty($email) || empty($phone)) {
        $error = "Name, email and phone cannot be empty.";
    } else {
        // Check email not taken by another student
        $chk = mysqli_prepare($conn,
            "SELECT id FROM users WHERE email = ? AND id != ?");
        mysqli_stmt_bind_param($chk, "si", $email, $student_id);
        ...
        if (mysqli_stmt_num_rows($chk) > 0) {
            ...
            $upd = mysqli_prepare($conn,
                "UPDATE users SET full_name=?, email=?, phone=?, dob=?, gender=?
                 WHERE id=?");
            mysqli_stmt_bind_param($upd, "sssssi",
                $full_name, $email, $phone, $dob, $gender, $student_id);
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['change_password'])) {
    ...
    mysqli_stmt_bind_param($chk, "is", $student_id, $current);
    mysqli_stmt_execute($chk);
    mysqli_stmt_store_result($chk);
    ...
    if (mysqli_stmt_num_rows($chk) == 0) {
        ...
        mysqli_stmt_bind_param($upd, "si", $new_pass, $student_id);
        mysqli_stmt_execute($upd);
...
<div class="main-content">
    ...
    <div class="page-heading">
        <h4><i class="fas fa-user me-2" style="color:#3498DB;"></i>My Profile</h4>
    </div>
    ...
    <div class="profile-card">
        ...
        <p style="color:#7f8c8d; font-size:13px; margin:4px 0 20px;">
